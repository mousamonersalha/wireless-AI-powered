...
def calculate():
    ...
    def validate_positive_number(name, value, int_flag=False):
        try:
            val = int(value) if int_flag else float(value)
            if val <= 0:
                raise ValueError
            return val
        except:
            raise ValueError(f"Invalid or missing positive value for '{name}'")

    try : 
        if scenario == "bitrate_pipeline":
    
            bandwidth = validate_positive_number("bandwidth", inputs.get("bandwidth"))
            quantizer_bits = validate_positive_number("quantizer_bits", inputs.get("quantizer_bits"), int_flag=True)
            source_encoder_rate = validate_positive_number("source_encoder_rate", inputs.get("source_encoder_rate"))
            channel_encoder_rate = validate_positive_number("channel_encoder_rate", inputs.get("channel_encoder_rate"))
            interleaver_bits = validate_positive_number("interleaver_bits", inputs.get("interleaver_bits"))
    ...
    ##############################################################################
            ...
            total_bandwidth = validate_positive_number("total_bandwidth", inputs.get("total_bandwidth"))              # Total system bandwidth in Hz
            subcarrier_spacing = validate_positive_number("subcarrier_spacing", inputs.get("subcarrier_spacing"))        # Spacing between subcarriers in Hz (e.g., 15 kHz)
            num_resource_blocks = validate_positive_number("num_resource_blocks", inputs.get("num_resource_blocks"), int_flag=True)        # Number of resource blocks 
            subcarriers_per_rb = validate_positive_number("subcarriers_per_rb", inputs.get("subcarriers_per_rb"), int_flag=True)          # Number of subcarriers in  RB
            modulation_order = validate_positive_number("modulation_order", inputs.get("modulation_order"), int_flag=True)              # Modulation order (M is 2 for QPSK, 4 for 16-QAM, etc.)
            coding_rate = validate_positive_number("coding_rate", inputs.get("coding_rate"))                      # Channel coding rate (0.5 for 1/2 rate coding)
    ...
    ##############################################################################
            ...
            freq_mhz = validate_positive_number("freq_mhz", inputs.get("freq_mhz"))                  # Frequency in MHz
            distance_km = validate_positive_number("distance_km", inputs.get("distance_km"))         # Distance between Tx and Rx in km
            ...
            received_power_dbm = effective_tx_power + tx_gain_dbi + rx_gain_dbi - rx_cable_loss_db - fspl
    ...
    ##############################################################################
            ...
            cluster_size = validate_positive_number("cluster_size", inputs.get("cluster_size"), int_flag=True)               # Cluster size (reuse factor, e.g., 3, 7)
            total_bandwidth = validate_positive_number("total_bandwidth", inputs.get("total_bandwidth"))       # Total system bandwidth in kHz
            channel_bandwidth = validate_positive_number("channel_bandwidth", inputs.get("channel_bandwidth"))   # Bandwidth per channel in kHz
            num_users = validate_positive_number("num_users", inputs.get("num_users"), int_flag=True)                     # Total number of users in the system
            traffic_per_user = validate_positive_number("traffic_per_user", inputs.get("traffic_per_user"))     # Offered traffic per user in Erlangs
